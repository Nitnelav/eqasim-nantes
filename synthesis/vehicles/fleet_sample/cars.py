...
def configure(context):
    ...
    context.stage("data.vehicles.critair.raw")
...
def _sample_vehicle(context, args):
    ...
    df_car_fleet_counts, df_car_age_counts = context.data("fleet"), context.data("age")

    commune_id = vehicle["commune_id"]

    if  commune_id in df_car_fleet_counts["commune_id"].unique():
        fleet = df_car_fleet_counts.loc[df_car_fleet_counts["commune_id"] == commune_id]
        choice = fleet.sample(weights="fleet")
        critair = choice["critair"].values[0]
        technology = choice["technology"].values[0]

        age_mask = (df_car_age_counts["critair"] == critair) & (df_car_age_counts["technology"] == technology)
        age = df_car_age_counts.loc[age_mask].sample(weights="fleet")["age"].values[0]
    else:
        choice = df_car_age_counts.sample(weights="fleet")
        critair = choice["critair"].values[0]
        technology = choice["technology"].values[0]
...
def execute(context):
    ...
    # in MATSim one must define a vehicle for every agent even if they don't have cars ?
    # df_persons = df_persons.loc[df_persons["car_availability"] != "none"]

    df_cars = pd.merge(df_persons[["household_id", "person_id"]], df_homes[["household_id", "commune_id"]], on = "household_id")

    df_cars = df_cars.rename(columns = { "person_id": "vehicle_id" })
    df_cars = df_cars.drop_duplicates("vehicle_id")
    df_cars["type_id"] = "default_car"

    df_car_fleet_counts, df_car_age_counts = context.stage("data.vehicles.critair.raw")

    res = []

    with context.progress(label = "Processing vehicles data ...", total = len(df_cars)) as progress:
        with context.parallel(dict(fleet = df_car_fleet_counts, age = df_car_age_counts)) as parallel:
            for df_partial in parallel.imap(_sample_vehicle, df_cars.to_dict(orient="records")):
                res.append(df_partial)

    df_cars = pd.DataFrame.from_dict(res)
